...
function device_changed(){
    ...
    $.ajax({
        ...
        success:function(data, status){
            owner_list = JSON.parse(data)
            $("#sel_chamber option").remove();
            for(i=0;i<owner_list.length;i++){
                //alert("<option value="+"'"+data_list[i][0]+"'"+">"+data_list[i][1]+"</option>")
                $("#sel_chamber").append("<option value="+owner_list[i]+">"+owner_list[i]+"</option>");
            }
...
function owner_changed(){
    var sel_device_id = document.getElementById("sel_device").value;
    var owner = document.getElementById("sel_chamber").value;
    $.ajax({
        url:'/data/history/owner',
        type:'post',
        data:{'dev_id': sel_device_id,
                'owner': owner},
        success:function(data, status){
